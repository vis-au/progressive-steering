...
class UseCaseTaxis(UseCase):
    def __init__(self):
        super().__init__(
            FILE_PATH, TABLE_NAME, X_ENCODING, Y_ENCODING, TRAINING_FEATURES
        )
    ...
    def send_info(self, eel: eel, column_names: List[str], cursor: Any):
        ...
        eel.send_dimension_total_extent(
            {"name": "passenger_count", "min": 0, "max": 192}
        )
        eel.send_dimension_total_extent(
            {"name": "trip_distance", "min": 0, "max": 189483.84}
        )
        ...
        eel.send_dimension_total_extent(
            {"name": "fare_amount", "min": -800, "max": 907070.24}
        )
        eel.send_dimension_total_extent({"name": "extra", "min": -80, "max": 96.64})
        eel.send_dimension_total_extent({"name": "mta_tax", "min": -80, "max": 150})
        eel.send_dimension_total_extent(
            {"name": "tip_amount", "min": -322.42, "max": 945.97}
        )
        eel.send_dimension_total_extent(
            {"name": "toll_amount", "min": -52.5, "max": 1650}
        )
        eel.send_dimension_total_extent(
            {"name": "improvement_surcharge", "min": -0.3, "max": 4000.3}
        )
        eel.send_dimension_total_extent(
            {"name": "total_amount", "min": -800.3, "max": 907071.04}
        )
