...
class UseCaseAirbnb(UseCase):
    ...
    def get_user_parameters(self, user_data):
        # user_data for distance contains only one value, but that one is a maximum, so make it a
        # range
        return {
    ...
    def send_info(self, eel, column_names, cursor):
        ...
        eel.send_dimension_total_extent(
            {"name": "zipcode", "min": 74400, "max": 750011}
        )
        eel.send_dimension_total_extent(
            {"name": "Saving opportunity", "min": -0.25, "max": 31.25}
        )
