preset_test_cases = (
    {}
)  # preset scenarios of selections in view space (get loaded from testCases.txt)


def get_box_data(test_case, cursor):
    qq = str(
        'SELECT * FROM listings WHERE price>=60 and price<=90 and "Saving opportunity"<='
        + str(test_case["boxMaxRange"])
        + ' and "Saving opportunity">='
        + str(test_case["boxMinRange"])
        + " and Distance>="
        + str(test_case["boxMinDistance"])
        + " and Distance<="
        + str(test_case["boxMaxDistance"])
    )
    cursor.execute(qq)
    ...
    qq = str(
        "SELECT * FROM listings WHERE price>=60 and price<=90 and abovem<="
        + str(test_case["boxMaxRange"])
        + " and abovem>="
        + str(test_case["boxMinRange"])
        + " and Distance>="
...
def get_test_cases():
    ...
    for i in range(len(preset_test_cases)):
        test_cases["testcase" + str(i + 1)] = {
            ...
            + str(i + 1)
            + "_"
            + str(preset_test_cases[i]["boxMinRange"])
            + "_"
            + str(preset_test_cases[i]["boxMaxRange"])
            + "_"
            + str(preset_test_cases[i]["boxMinDistance"])
            + "_"
            + str(preset_test_cases[i]["boxMaxDistance"]),
            "x_bounds": [
                preset_test_cases[i]["boxMinRange"],
                preset_test_cases[i]["boxMaxRange"],
            ],
            "y_bounds": [
                preset_test_cases[i]["boxMinDistance"],
                preset_test_cases[i]["boxMaxDistance"],
            ],
        }
    return test_cases
