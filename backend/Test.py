...
def testGenerator(case=c):
    ...
    for i in range (1):
        log={}
        a=random.randint(0,case['range'][1]-case['range'][0])
        b=random.randint(0,case['range'][1]-case['range'][0])
        boxMinRange=min(a,b)
        boxMaxRange=max(a,b)
        if boxMinRange==boxMaxRange:
            boxMaxRange+=1
        
        a=random.randint(0,case['MaxDistance'])
        b=random.randint(0,case['MaxDistance']+5)
        boxMinDistance=min(a,b)
        boxMaxDistance=max(a,b)
        if boxMinDistance==boxMaxDistance:
            boxMaxDistance+=1
        #################       
        boxMinRange=15
        boxMaxRange=40
        ...
        query+="price >="+str(case['range'][0])+" AND price <="+str(case['range'][1])+ " AND distance >="+str(boxMinDistance)+" AND distance <="+str(boxMaxDistance)+" AND abovem >="+str(boxMinRange)+" AND abovem <="+str(boxMaxRange) 
        mycursor = mydb.cursor()
        mycursor.execute(query)
        myresult = mycursor.fetchall()
        for x in myresult:
        ...
        log[i+1]={'GT':GT,'boxMinRange':boxMinRange,'boxMaxRange':boxMaxRange,'boxMinDistance':boxMinDistance,'boxMaxDistance':boxMaxDistance,'price':case['range'],'chunks':[]}
        

        logFileName='x_'+str(chunkSize)+'_'+str(minimumBoxItems)+'.txt'
        print(logFileName)
        logM={}
        logM={'totalQueryElements':len(GT),'totalIN':len(IN),'totalOUT':len(OUT),'chunks':{}}

        doRun(GT,IN,OUT,case,log[i+1]['chunks'],logM['chunks'],minimumBoxItems,chunkSize,True) #using tree
        f=open(logFileName,'w',encoding="UTF8")
        ...
        f=open("log_"+str(list(log.keys())[0])+"_Tree.txt",'w',encoding="UTF8")
        
        print(str(log),file=f)
        
        
        f.close()
        doRun(GT,IN,OUT,case,log[i+1]['chunks'],minimumBoxItems,chunkSize,False) #not using tree
        f=open("log_"+str(list(log.keys())[0])+"_noTree.txt",'w',encoding="UTF8")
        print(str(log),file=f)
        f.close()
        '''
...
def doRun(GT,IN,OUT,case,log,logM,minimumBoxItems,chunkSize,useTree):
    ...
    feedTuples(case,sql,log,logM,useTree,minimumBoxItems,chunkSize,GT,IN,OUT)   
    pass
...
global totalDiz
def feedTuples(case,query,log,logM,useTree,minimumBoxItems,chunkSize,GT=None,IN=None,OUT=None):
    global modifier
    global DIZ_plotted
    global treeReady #it is used to interrupt the main chunking cycle
    global x
    ...
    if len(myresult)>0:
        while state != 'empty': #len(myresult)>0:#not treeReady or True:
             ...
             if len(IN)>0:
                 ...
                 print('Chunk:',chunks,state, 'PRECISION=', actualIn/chunkSize,'RECALL=', len(INplotted), len(INplotted)/len(IN),modifier[0:20],'... marco true_positive = ', marcoMetrics['true_positive'])
...
f=open('DIZ_plotted_test.txt',)


fft=eval(open("log_"+str(1)+"_Tree.txt",'r',encoding="UTF8").read())

ffnt=eval(open("log_"+str(1)+"_noTree.txt",'r',encoding="UTF8").read())

m=eval(open('x_100_50.txt','r',encoding="UTF8").read())

DPT=eval(open('DIZ_plotted_test.txt','r',encoding="UTF8").read())
DPW=eval(open('DIZ_plotted_web.txt','r',encoding="UTF8").read())
